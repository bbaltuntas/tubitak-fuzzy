...
class Variable:
    ...
    def init_range(self, start, end):
        ...
        self.end = end
        if self.type == "input":
            self.variable_ctrl = ctrl.Antecedent(np.arange(start, end + 1, 1), self.name)

        elif self.type == "output":
            self.variable_ctrl = ctrl.Consequent(np.arange(start, end + 1, 1), self.name)
        # self.variable = np.arange(start, end+1, 1)
